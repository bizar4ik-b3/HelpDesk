...
namespace HelpDeskTrain.Controllers
{
    ...
    public class RequestController : Controller
    {
        ...
        {
            User user = db.Users.Where(m => m.Login == HttpContext.User.Identity.Name).First();
            var requests = db.Requests.Where(r => r.User.DepartmentId == user.DepartmentId)
                                        .Include(r => r.Category)  // добавляем категории
                                        .Include(r => r.Lifecycle)  // добавляем жизненный цикл заявок
                                        .Include(r => r.User)         // добавляем данные о пользователях
                                        .OrderByDescending(r => r.Lifecycle.Opened); // упорядочиваем по дате по убыванию   
        ...
        {
            User ModeratorUser = db.Users.Where(m => m.Login == HttpContext.User.Identity.Name).FirstOrDefault();
            ...
            var requests = db.Requests.Include(r => r.User)
                                    ...
                                    .Where(r => r.User.DepartmentId == ModeratorUser.DepartmentId);
            List<User> executors = db.Users.Include(e => e.Role)
                .Where(e => e.Role.Name == "Исполнитель")
                .Where(e=>e.DepartmentId==ModeratorUser.DepartmentId)
                .ToList<User>();
       ...
       ////////////////////////////////////////
        ...
        [HttpPost]
        public ActionResult Edit(Request request, HttpPostedFileBase error)
        {
            if (ModelState.IsValid)
            {
  
                db.Entry(request).State = EntityState.Modified;
                db.Entry(request).Property(x => x.UserId).IsModified = false;
                db.Entry(request).Property(x => x.Status).IsModified = false;
                db.Entry(request).Property(x => x.LifecycleId).IsModified = false;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            User user = db.Users.Where(m => m.Login == HttpContext.User.Identity.Name).FirstOrDefault();
